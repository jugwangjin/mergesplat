...
- **비용**: **primitive color**: 전체 SH 계수 L2 (view-dependent 반영; coeff 비슷할 때만 merge). primitive normal (1−cos), GT rgb/depth/normal (multi-view 평균), **normal-dist** \|(μ_u−μ_v)·n_avg\| / major_extent (장축 extent로 정규화), **depth** (scale-dependent when scales given), **scale_diff**: rotations 주어지면 tangent-plane 2D **Bhattacharyya** (Cost = 1−BC). rotations 없으면 \|r_u−r_v\|/(r_u+r_v).
